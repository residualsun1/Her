...
test("starter preview is removed and project modules are present", async () => {
  ...
  assert.match(particle, /organicBoundary/);
  assert.match(particle, /organicMask/);
  assert.match(particle, /haloChance = clamp\(outer \* 0\.62 \+ edge \* \(1 - coreProtection\) \* 0\.34/);
  assert.doesNotMatch(particle, /analyzeSubject|backgroundSuppression|aSubject/);
  assert.match(particle, /pointBudget \* 0\.32/);
  assert.match(particle, /precomposed/);
  assert.doesNotMatch(particle, /trailCanvasRef/);
  assert.doesNotMatch(particle, /destination-out/);
