...
test("product modules and critical interaction contracts are present", async () => {
  ...
  assert.equal((herApp.match(/<ThinkingOrb/g) ?? []).length, 2);
  ...
  assert.match(herApp, /QWEN_SPEECH_REQUEST_TIMEOUT_MS = 8_000/);
  assert.match(herApp, /QWEN_SPEECH_FIRST_CHUNK_TIMEOUT_MS = 6_000/);
  assert.match(herApp, /QWEN_SPEECH_START_DEADLINE_MS = 14_000/);
  ...
  assert.match(herApp, /setTurns\(\(items\) => \[\.\.\.items, assistantTurn\]\);[\s\S]*?setReplyState\("ready"\);[\s\S]*?speak\(text, undefined, 0, "zh", preparedReply\.value\.audio\)/);
  ...
  assert.doesNotMatch(herApp, /speechSynthesis|SpeechSynthesisUtterance|fallbackToBrowserVoice/);
  assert.match(herApp, /provider: "qwen"[\s\S]*?responseProvider !== "qwen"/);
  assert.match(herApp, /for \(let attempt = 0; attempt < 2; attempt \+= 1\)[\s\S]*?prepareSynthesizedSpeech\(text, spokenLanguage, true\)/);
  assert.match(herApp, /const playStreamingPcm[\s\S]*?QWEN_SPEECH_FIRST_CHUNK_TIMEOUT_MS[\s\S]*?getInt16\(index \* 2, true\)[\s\S]*?await readFirstChunk\(\)/);
  assert.match(herApp, /qwenStartTimer = window\.setTimeout\([\s\S]*?QWEN_SPEECH_START_DEADLINE_MS/);
  assert.match(herApp, /preferStreaming && response\.body[\s\S]*?kind: "pcm-stream"/);
  assert.match(herApp, /const waitingForAssistant =[\s\S]*?turns\.at\(-1\)\?\.role !== "assistant"/);
  assert.match(herApp, /!sentEcho && replyState !== "holding" && !waitingForAssistant && currentAssistant/);
  assert.match(herApp, /speak\(welcome, undefined, 0, "zh", undefined, \(\) => \{[\s\S]*?setConversationChromeVisible\(true\)/);
  assert.match(herApp, /speak\(text, undefined, 0, "zh", preparedReply\.value\.audio\)/);
