...
test("product modules and critical interaction contracts are present", async () => {
  ...
  assert.match(herApp, /SPEECH_PREPARE_TIMEOUT_MS = 5_000/);
  ...
  assert.match(herApp, /setTurns\(\(items\) => \[\.\.\.items, assistantTurn\]\);[\s\S]*?setReplyState\("ready"\);[\s\S]*?Promise\.race<PreparedSpeechAudio \| null>[\s\S]*?preparedReply\.value\.audio[\s\S]*?speak\(text/);
  ...
  assert.match(herApp, /prepareSynthesizedSpeech\(text, spokenLanguage, true\)[\s\S]*?SPEECH_PREPARE_TIMEOUT_MS[\s\S]*?fallbackToBrowserVoice\(\)/);
