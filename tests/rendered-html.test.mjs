...
test("starter preview is removed and project modules are present", async () => {
  ...
  assert.match(gpu, /uWebTension/);
  assert.match(gpu, /uWebFrequency/);
  assert.match(gpu, /webBridge/);
  assert.match(gpu, /webProfile/);
  ...
  assert.match(particleConfig, /dreamRimWidth: 0\.2/);
  assert.match(particleConfig, /webReach: 1\.15/);
  assert.match(particleConfig, /webTension: 0\.62/);
  assert.match(particleConfig, /webFrequency: 5\.2/);
