...
- 桌面端按画布面积使用约 68,000–132,000 个表面点，低性能设备自动降低预算；点径接近采样间距，使人脸、发型、衣领和场景轮廓由粒子本身连续重建。
- 暗部不再因“颜色亮度 × 透明度”被二次压暗；眼睛、头发和阴影仍保留足够粒子密度。
- 点云具有伪深度：图片亮度、低频 Depth Wave 和音频共同生成 Z 坐标。按住拖动时，整张点云进行 3D 俯仰/侧转，松手后弹性回正。

**B. 自适应图像连续性层**

- 与点云相同的 `contain + 0.88 fit` 下保留一层默认透明度 0.26 的低频结构，使静止状态下的人物和物体能立即辨认。
- 指针增强和 3D 拖拽时，该层会按交互强度自动衰减至最低约 16%，让黑洞、网格和空间变形重新成为视觉主体；`Image continuity` 提供 0.10–0.46 的可调范围。
...
| 图片粒子化基础 | 已实现以高密度 WebGL2 点云为主体的粒子画布：默认 0.26 的自适应连续性层保证主体可辨识，交互时自动衰减；包含 15% Halo 预算、椭圆溶解、低频流场、黑色引力井、亮环、漩涡、整张点云拖拽旋转、弹性回正、声音深度波、自适应粒子数、`Image continuity` 调节和 reduced-motion 降级。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已在高密度粒子成像、3D Drag/Tilt、Gravity Well、亮环、Swirl、Halo 和音频深度波之上，将静止连续性校准至 0.26、Halo 收束至 15%，并让底层随交互自动淡出，避免再次落入“纯照片”或“纯粒子”两个极端。 | 当前使用自定义 WebGL2 实现并已通过生产构建与静态回归；仍需用 `raw.png` 在真实浏览器完成静止、拖拽、强音频和指针划过五官四种肉眼验收。独立 Bloom、真正的反馈纹理拖尾和同图同音频录屏尚未完成，因此不能称为 1:1 复刻。 |
