...
### 4.2 V4 采用“粒子主体优先”的四层合成

**A. 主体浅粒子纹理层**

- 桌面端按画布面积使用约 68,000–132,000 个表面点，低性能设备自动降低预算；中央主体点径更小、透明度约为外围的三分之一，形成可见但克制的网格/粒子纹理。
- 暗部不再因“颜色亮度 × 透明度”被二次压暗；眼睛、头发和阴影仍保留足够粒子密度。
- 从主体中心向轮廓过渡时，粒子透明度、尺寸、流场和音频位移连续增强；点云仍具有伪深度并支持 3D 拖拽与弹性回正。

**B. 主体连续结构层**

- 与点云相同的 `contain + 0.88 fit` 下保留一层默认透明度 0.52 的低频原图结构，并用椭圆遮罩向轮廓和外围渐隐，使人物与物体能立即辨认。
- 指针增强和 3D 拖拽时，该层只轻度衰减，不低于当前清晰度的约 54%，使黑洞、网格和空间变形可见的同时仍保留主体辨识；`Subject clarity` 提供 0.34–0.70 的可调范围。
- 已经粒子化的参考成品标记为 `precomposed`，连续底图与约 94% 强度的动态粒子场并存，并叠加独立时间衰减拖尾；不再以降低整个粒子画布透明度的方式规避双重粒子化。

**C. 边缘溶解与外围 Halo 层**

- 使用不规则椭圆/SDF 遮罩定义“记忆入口”。
- 只在人物保护区外和画面外围发射粒子；眼睛、鼻子、嘴等内部亮度边缘不能直接产生远距离 Halo。
- Halo 上限约为核心预算的 15%；径向扩散、切向旋流和随机方向共同形成视频中的蓝白粒子带，同时避免静止状态吞没主体。
...
- Bloom 只让超过阈值的亮点泛光。
- Additive Blending 用于粒子、亮环和 Halo；极弱连续性层使用普通 Alpha。
- Vignette 压暗画面边缘；Tone Mapping 保护高光层次。
- 必要时加入轻量反馈纹理形成短拖尾，但默认不使用长时间运动模糊。
...
| 图片粒子化基础 | 已实现三层 WebGL2 合成：中央主体以 0.52 连续结构保证可辨识，完整粒子场在中心保持较小形变、向轮廓/外围过渡为高位移粒子与 20% Halo，独立反馈画布产生真实时间衰减拖尾；已粒子化参考素材使用 precomposed 双层保真模式。Gravity Well、亮环、漩涡、3D 拖拽、弹性回正、声音深度波和 reduced-motion 均保留。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已确认 `example-1.png`、`example-3.png` 是目标粒子成品而非原始输入；内置参考素材改为“连续底图 + 强动态粒子 + 时间拖尾”，普通上传图同样采用主体保真与边缘强粒子的连续空间分区。3D Drag/Tilt、Gravity Well、亮环、Swirl 和声音响应保持完整强度。 | 当前使用自定义 WebGL2 实现，反馈画布已加入真正的时间衰减拖尾；仍需用真正原图在真实浏览器完成静止、拖拽、强音频和指针划过主体四种肉眼验收。独立后处理 Bloom 和同图同音频录屏尚未完成，因此不能称为 1:1 复刻。 |
