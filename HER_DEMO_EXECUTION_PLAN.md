...
- 指针增强和 3D 拖拽时，该层会按交互强度自动衰减，让黑洞、网格和空间变形重新成为视觉主体；`Subject clarity` 提供 0.34–0.70 的可调范围。
- 已经粒子化的参考成品标记为 `precomposed`，只叠加约 24% 的动态粒子层，不再执行完整转换，避免双重粒子化。
...
| 图片粒子化基础 | 已实现分区 WebGL2 粒子画布：中央主体以 0.52 连续结构和浅粒子纹理保证可辨识，向轮廓/外围逐渐过渡为高透明度、高位移粒子与 15% Halo；已粒子化参考素材使用 precomposed 轻量模式，避免重复处理。其余 Gravity Well、亮环、漩涡、3D 拖拽、弹性回正、声音深度波和 reduced-motion 均保留。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已确认 `example-1.png`、`example-3.png` 是目标粒子成品而非原始输入；内置参考素材停止重复粒子化，普通上传图改为“中央浅纹理/高结构保真—轮廓渐强—外围强 Halo”的连续空间分区。3D Drag/Tilt、Gravity Well、亮环、Swirl 和声音响应保持不变。 | 当前使用自定义 WebGL2 实现并已通过生产构建与静态回归；仍需用真正原图在真实浏览器完成静止、拖拽、强音频和指针划过主体四种肉眼验收。独立 Bloom、真正的反馈纹理拖尾和同图同音频录屏尚未完成，因此不能称为 1:1 复刻。 |
