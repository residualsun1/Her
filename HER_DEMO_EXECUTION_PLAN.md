...
- 按画布面积和设备能力使用约 60,000–280,000 个点；基础网格只占预算的一部分，主体中心再按亮度与局部边缘生成 2–4 倍抖动采样，所以不依赖连续照片也能辨认。
- 主体点与 Halo/轮廓点按连续区间写入同一 Buffer，并分两次绘制：主体使用 `SRC_ALPHA / ONE_MINUS_SRC_ALPHA` 保留暗部层次，轮廓和 Halo 使用 Additive Blending 产生光效。
- 暗像素根据局部梯度进行结构性抬底，而不是统一漂白；眼睛、头发、阴影、亭柱与屋檐等边缘另有独立的暖白轮廓粒子。
...
- Halo 上限约为核心预算的 20%，轮廓粒子上限约 14%；暖白为主、少量冷色漂移的细小粒子通过径向扩散、切向旋流和随机方向共同形成星云带。
- 使用 Simplex Noise 与 Curl Noise 让边缘像被气流剥离，而不是机械地向外爆炸。

**D. 交互场、声音响应与后期合成层**

- 指针中心使用纯排斥空洞，外围保留切向 Swirl、高亮环和传播波纹；不再用宽范围径向吸引抵消黑洞效果。
- 点阵基础深度加入轻量球面穹顶；拖拽幅度超过阈值后，点云会连续折叠成流体光带，并在松手后随阻尼回到可辨识形态。
- 用户说话、AI 发声和音乐增加深度波、外围呼吸、粒径和闪光；主体中心仍保留较低但可见的运动量。
- 少量独立高亮点在外部黑场缓慢漂移，并在音乐瞬态或说话重音时短暂增亮。
- Bloom 只让超过阈值的亮点泛光。
- 普通 Alpha Blending 用于主体重构，Additive Blending 仅用于轮廓、亮环和 Halo；独立 2D 反馈画布每两帧采样当前 WebGL 结果并逐渐擦除，形成真正的时间衰减拖尾。
...
| 图片粒子化基础 | 已实现单一高密度 WebGL2 粒子图像：连续原图只作为 WebGL 失败时的降级内容；主体中心 2–4 倍过采样，暗部结构提亮并由普通 Alpha 混合重构，独立暖白轮廓与 20% Halo 使用 Additive 绘制。外围是不规则柔化包络，反馈画布产生时间衰减拖尾；纯排斥黑洞、传播波纹、漩涡、球面穹顶、拖拽光带、弹性回正、声音深度波和 reduced-motion 均保留。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已确认 `example-1.png`、`example-3.png` 是目标粒子成品而非原始输入；正常渲染取消可见底图，普通上传图由单一粒子平面完成主体保真、暗部重塑、独立轮廓、无边框溶解与暖白 Halo。视频中的十项参数均为实时 Uniform；大拖拽新增光带折叠，鼠标场改为纯排斥黑洞。 | 当前使用自定义 WebGL2 实现；仍需用凉亭、圣诞树和原始人像在真实浏览器完成静止、拖拽、强音频、参数极值和指针划过主体五种肉眼验收。独立频谱柱、后处理 Bloom、粒子隧道转场和同图同音频录屏尚未完成，因此不能称为 1:1 复刻。 |
