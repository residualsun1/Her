...
- 将上传图片绘制到离屏 Canvas，做 `cover` 裁切。
- 按规则网格采样图片，生成一张真正具有 `x / y / z` 坐标的高密度点云平面；不是在实体图片上覆盖一层装饰粒子。
- 每个点从图片 UV 读取颜色；以亮度、边缘梯度和不规则遮罩决定透明度、尺寸及是否保留。
- Z 坐标由图片亮度深度、低频程序化 Depth Wave 和音频位移共同组成；整张点云再进行带阻尼的 3D 俯仰与侧翻。
- 对亮度、色相和边缘梯度做采样，暗部减少粒子，亮部保留高光。

**B. 边缘溶解层**

- 使用不规则椭圆/SDF 遮罩定义“记忆入口”。
- 遮罩边缘进行高密度粒子发射；越靠外透明度越低、扰动越大。
...
- Bloom 只让超过阈值的亮点泛光。
- Additive Blending 让相邻亮粒子叠加出能量感。
...
| 图片粒子化基础 | 已实现依赖 WebGL2 的图片采样粒子画布，包含核心画像、外围光环、低频流动、指针局部力场、音量驱动、自适应粒子数和 reduced-motion 降级。用户上传 PNG/JPEG/WebP 后可直接生成粒子画像。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 粒子画像、外围溶解感、黑场、鼠标力场和声音驱动已经可见。 | 当前使用自定义 WebGL2 实现，尚未完成参考视频中的完整 Global Tilt、侧翻、丝带式 Depth Wave、独立 Bloom 后处理、拖尾和全参数 Advanced 面板；也尚未做同图同音频对照录屏，因此不能称为 1:1 复刻。 |
