...
- 将上传图片以与粒子画布完全一致的 `contain + 0.88 fit` 放置，使用中心椭圆遮罩向黑场渐隐。
- 该层采用普通 Alpha 混合，保持静止且不响应声音或鼠标，负责保留眼鼻嘴、发型、衣领和姿态等低频身份特征。
- 默认有效亮度约保留原图的 55%–62%；用户可通过 `Image clarity` 在写实与抽象之间微调。

**B. 图内细粒子纹理层**

- 按规则网格采样图片，生成具有 `x / y / z` 坐标的高密度点云平面。
- 每个点从图片读取颜色；亮度、边缘梯度和随机种子控制透明度、尺寸与闪烁。
- 中央人物保护区降低抖动、音频位移、鼠标位移和粒径，让粒子提供网格/扫描质感，而不再独自承担整张图片重建。
- Z 坐标由低频 Depth Wave 和音频位移共同组成；后续仍可加入带阻尼的 3D 俯仰与侧翻。

**C. 边缘溶解与外围 Halo 层**

- 使用不规则椭圆/SDF 遮罩定义“记忆入口”。
- 只在人物保护区外和画面外围发射粒子；眼睛、鼻子、嘴等内部亮度边缘不能直接产生远距离 Halo。
- Halo 控制在核心点数约 8%–12%；越靠外透明度越低、扰动越大。
- 使用 Simplex Noise 与 Curl Noise 让边缘像被气流剥离，而不是机械地向外爆炸。

**D. 后期合成与漂浮火花层**

- 少量独立高亮点在外部黑场中缓慢漂移。
- 只在音乐瞬态或说话重音时短暂增亮，避免画面持续嘈杂。
- Bloom 只让超过阈值的亮点泛光。
- Alpha Blending 用于稳定图像与暗部；Additive Blending 只用于高亮粒子和 Halo。
...
| 图片粒子化基础 | 已实现依赖 WebGL2 的混合粒子画布：稳定原图核心保留人物细节，图内细粒子提供网格质感，外围 Halo 负责溶解与舞动；同时包含中央动态保护、低频流动、指针局部力场、音量驱动、自适应粒子数、`Image clarity` 调节和 reduced-motion 降级。用户上传 PNG/JPEG/WebP 后可直接生成粒子画像。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已根据 `raw.png → particle.png` 迭代为低频原图保真层、表面粒子层和外围 Halo 层；Halo 上限降至约 10%，并限制中央人脸的流场、音频和鼠标位移。 | 当前使用自定义 WebGL2 实现，已通过生产构建与静态回归，但本轮浏览器自动化连接未能完成，因此仍需人工使用 `raw.png` 检查静止、强音频和鼠标经过三种状态；完整 Global Tilt、侧翻、丝带式 Depth Wave、独立 Bloom、拖尾和同图同音频录屏也尚未完成，不能称为 1:1 复刻。 |
