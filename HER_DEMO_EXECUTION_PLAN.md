...
- 与点云相同的 `contain + 0.88 fit` 下保留一层默认透明度仅 0.06 的暗部连续性保险。
- 它不承担主要成像，也不能压过网格、粒子、引力井或 3D 拖拽；`Image continuity` 只为特别暗的照片提供 0–0.22 的小范围补偿。

**C. 边缘溶解与外围 Halo 层**

- 使用不规则椭圆/SDF 遮罩定义“记忆入口”。
- 只在人物保护区外和画面外围发射粒子；眼睛、鼻子、嘴等内部亮度边缘不能直接产生远距离 Halo。
- Halo 上限约为核心预算的 18%；径向扩散、切向旋流和随机方向共同形成视频中的蓝白粒子带。
...
| 图片粒子化基础 | 已实现以高密度 WebGL2 点云为主体的粒子画布：粒子重建人物与场景，极弱连续性层只补暗部；包含 18% Halo 预算、椭圆溶解、低频流场、黑色引力井、亮环、漩涡、整张点云拖拽旋转、弹性回正、声音深度波、自适应粒子数、`Image continuity` 调节和 reduced-motion 降级。 |
...
| 模块 | 已有能力 | 仍缺内容 |
|---|---|---|
| 参考视觉复现 | 已重新逐帧分析 `voice-interaction-1.mp4`，确认并实现高密度粒子成像、3D Drag/Tilt、带阻尼回正、Gravity Well、亮环、Swirl、强 Halo、音频深度波与闪光；不再用高透明度静态照片遮盖粒子。 | 当前使用自定义 WebGL2 实现并已通过生产构建与静态回归；仍需用 `raw.png` 在真实浏览器完成静止、拖拽、强音频和指针划过五官四种肉眼验收。独立 Bloom、真正的反馈纹理拖尾和同图同音频录屏尚未完成，因此不能称为 1:1 复刻。 |
