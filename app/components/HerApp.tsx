...
export function HerApp() {
  ...
  const [danceStrength, setDanceStrength] = useState(0.72);
  const [imageClarity, setImageClarity] = useState(0.76);
    ...
    <main className={styles.app}>
        ...
        <section className={styles.galleryPage}>
          ...
          <div className={styles.gardenCarousel}>
            ...
            <div className={styles.centerMemory} onClick={openGardenConversation} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openGardenConversation(); } }} role="button" tabIndex={0}>
              <ParticleGarden imageUrl={gardenItems[gardenIndex].imageUrl} audioLevel={0.08} interactionStrength={0.7} imageClarity={0.68} />
      ...
      {settingsOpen && (
        <aside className={styles.settingsPanel} aria-label="Settings">
          ...
          <label>Image clarity <output>{imageClarity.toFixed(2)}</output><input type="range" min="0.5" max="0.92" step="0.02" value={imageClarity} onChange={(event) => setImageClarity(Number(event.target.value))} /></label>
