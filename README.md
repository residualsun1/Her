...
- **记忆**：以黑场横向回廊浏览示例图片和本机上传过的图片；中央粒子画像可以重新打开并继续一段对话。
- **回廊**：以卡片查看已保存对话、总结、逐轮文字和可用音频；也可以切到日历，把刚保存或当前选中的本地记忆钉到某一天。
- **音乐**：从本机选择多首 `.mp3` 或 `.flac` 背景音乐，通过可折叠音乐列表随时切换；播放时会用音量驱动粒子舞动。音乐只服务于当前页面会话，不会自动上传或永久保存。
...
当前视觉不再显示连续原图底层。上传图片会被采样为约 68,000–132,000 个 WebGL 点，由同一个高密度粒子平面重构主体；中心区域通过低位移、高颜色保真和较密的点阵保持可辨识，径向不规则遮罩使粒子在轮廓和四周逐渐扩散并消失，因此不会留下图片矩形边框。独立的时间衰减层在移动、拖拽、漩涡和声音响应时留下光迹；指针会形成引力井、亮环与传播波纹，按住拖动可让整张粒子平面产生 3D 俯仰/侧转。
...
- 在 `live` 模式下，文字对话、翻译和总结会发送给所选供应商；开启图片理解后，图片副本会发送给 Qwen。API Key 只应放在服务端环境变量中，绝不能写入前端代码或使用 `NEXT_PUBLIC_*`。
...
app/
  ...
  api/                     Chat、翻译、总结、图片理解、ASR/TTS 状态接口
...
视觉和权限仍需人工在真实浏览器中验证：至少测试一次图片上传、鼠标力场、麦克风允许/拒绝、文字输入、保存记忆、刷新后的 IndexedDB 恢复、记忆切换、回廊日历，以及本地音乐播放。参考视频没有提供原始 Shader、音频或专属音色元数据，所以当前版本不应被描述为已经完成 1:1 视觉验收。
