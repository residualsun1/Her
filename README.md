...
当前视觉使用空间分区：中央主体保留默认 0.52 的连续原图结构，只覆盖较浅、较细的粒子纹理；向轮廓和画面边缘过渡时，原图逐渐淡出，粒子密度、位移、溶解和 Halo 明显增强。指针会形成引力井、亮环和漩涡，按住拖动可让整张点云产生 3D 俯仰/侧转，声音和音乐继续驱动深度波与闪光。交互增强时中央连续层会自动减弱。设置中的 `Subject clarity` 可在 0.34–0.70 之间微调。

`public/demo` 中的内置素材本身就是已经完成粒子化的视觉参考，因此现在会以 `precomposed` 模式轻量展示，不再进入完整粒子化管线。用户新上传的普通原图才会执行上述“主体浅、边缘强”的转换。
