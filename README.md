...
当前视觉使用三层合成：中央主体保留默认 0.52 的连续图像结构；完整强度的高密度粒子场覆盖其上，中心粒子形变较小、边缘位移和 Halo 更强；独立的时间衰减层在移动、拖拽、漩涡和声音响应时留下可见光迹。指针会形成引力井、亮环和漩涡，按住拖动可让整张点云产生 3D 俯仰/侧转，声音和音乐继续驱动深度波与闪光。交互时连续层只轻度减弱，保证主体仍能辨认。设置中的 `Subject clarity` 可在 0.34–0.70 之间微调。

`public/demo` 中的内置素材本身就是已经完成粒子化的视觉参考；`precomposed` 模式会保留其连续底图，同时恢复接近完整强度的动态粒子、拖尾、漩涡和鼠标扰动，避免上一版退化为静态原图。用户新上传的普通原图同样使用“清晰底层 + 动态粒子场 + 时间拖尾”的三层转换。
