...
右上角设置面板可以切换 AI Brain、回复语言、浏览器音色风格、图片理解开关、本机原声保存开关、粒子舞动强度、图像连续性和鼠标力场强度。
...
当前视觉由高密度点云重建主体，并叠加默认 0.26 的低频连续性层：静止时先保证人物和物体可辨识，粒子网格、外围 Halo 与辉光仍清晰可见。指针经过画面会形成引力井、亮环和漩涡，按住并拖动可以让整张点云产生 3D 俯仰/侧转，松手后带阻尼回正；交互增强时连续性层会自动变淡，避免填平黑洞和粒子变形。`Image continuity` 可在 0.10–0.46 之间微调。
